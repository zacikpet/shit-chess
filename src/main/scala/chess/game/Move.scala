...
object Move {
  def fromString(str: String): Move = {
    if (str.length != 5) throw new IllegalArgumentException("Move must be 5 characters long")

    if (str(2) != ':') throw new IllegalArgumentException("Move must be in format 'a1-a2'")
